...
def setup_llm(resume_content):
    ...
    prompt_template = PromptTemplate.from_template(
        f"""You are AniPro - a personalized AI chatbot designed to assist visitors on Aniketh's portfolio website. 
        Use the following resume content to provide responses about Aniketh's background, skills, experience, and projects:
        {resume_content}

        Start by greeting visitors and offering to answer questions about Aniketh's profile. Always provide concise, accurate, and professional responses. 
        If the user asks about something outside the resume content, politely state that you can only answer based on the provided information. But restrict yourself from giving Aniketh's contact info - when asked for it, give Aniketh's portfolio website link and ask to contact Aniketh via its contact section on the website.
...
def main():
    ...
    # Debugging: Check files in the deployed environment
    st.write("Files in directory:", os.listdir())

    # Extract resume content
    resume_path = "data.pdf"  # Ensure this file exists in your repo
    if os.path.exists(resume_path):
        resume_content = extract_resume_content(resume_path)
    else:
        st.error("Resume file not found in the deployment environment.")
        return
    ...
    with st.sidebar:
        ...
        This chatbot is here to assist you with any questions about Aniketh, Aniketh's skills, experience, or projects. Its responses are limited to Aniketh's personal profile. For more visuals and concise responses, please visit the chatbot at the bottom right corner of the portfolio page.
        ...
        - AniPro2.0 cannot answer any personal questions about Aniketh; please refrain from asking such questions!
